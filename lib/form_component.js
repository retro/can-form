...
'can/construct/proxy/'
	...
	var FormControl = can.Control.extend({
		...
		"inserted" : function(){
			this.__subtemplateId = this.scope.attr('map')._cid;
		},
		...
		destroy : function(){
			delete SUBTEMPLATE_CACHE[this.__subtemplateId];
	...
	var wrapSubtemplateInFormScope = function(subtemplate, subtemplateRegistry){
		var isTopLevel;
		...
		var res = function(){
			...
			return wrapped({}, {
				...
				__subtemplate : function(opts){
					var subtemplateId = opts.context._cid;

					if(!SUBTEMPLATE_CACHE[subtemplateId]){
						SUBTEMPLATE_CACHE[subtemplateId] = subtemplate(opts.scope, tagsHelpers);
					}
					return SUBTEMPLATE_CACHE[subtemplateId]
				}
			});
		}
	...
	var componentOpts = {
		setup : function(el, hookupOptions){
			...
			if(isTopLevel){
				...
				opts.__form = true;
				opts.__errors = (function(){
					var scopeErrors;
					return function(){
						if(!scopeErrors){
							scopeErrors = scope.attr(getPath(el.getAttribute('errors'))) || can.compute();
						}
						return scopeErrors;
					}
				})();
			...
			if(!hookupOptions.scope._parent){
				hookupOptions.scope = scope.add(opts);
			} else {
			...
			this._formControl = new FormControl(this._control.element, {
				scope : this.scope
			})

			// If we are in the top level form, add some objects to the control instance
			if(opts.__form){
				this._formControl.__form      = opts.__form;
				this._formControl.__errors    = opts.__errors;
				this._formControl.validations = validations;
				this._formControl.dirtyAttrs  = dirtyAttrs;
				this._formControl.bindErrors(opts.__errors);
				this._formControl.scope.attr('state', FormStates.CLEAN);
				this._formControl.scope.setErrors = function(errors){
